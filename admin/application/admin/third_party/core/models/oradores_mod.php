...
class oradores_mod extends RR_Model {
    var $atributo       = 'oradores';
    var $table          = 'oradores';
    var $module         = 'oradores';
    var $module_title   = "Oradores";    
    var $id;
    var $eid;
    ...
    public function listado(){    
        ...
        $datagrid["columns"][] = array("title" => "Orden", "field" => "order");
        $datagrid["columns"][] = array("title" => "Status", "field" => "status", 'format'=>'icon-activo');        
        #CONDICIONES & CACHE DE CONDICIONES     
        $this->db->start_cache();   
        $this->db->select('o.id, o.nombre, o.cargo, o.status, o.order', false);
        $this->db->where('o.evento_id =',$this->eid);   
    ...
    public function setOrder(){
        $data = array();
        foreach($_POST['dg-tr'] as $k => $v){
            $data[$k] = array('id'=>$v, 'order'=>$k);
        }
        
        
        $upd = $this->db->update_batch('oradores', $data, 'id');
        
        $success = true;
        $responseType = 'function'; 
        $function     = 'reloadTable';        
        $status       = $this->view('alerts/generic', array('success'=>'Registro Ordenado Exitosamente', 'type'=>'success'));                    
