...
class eventos_mod extends RR_Model {
    ...
    */
    public function getOradores(){
        $result = $this->db->order_by('order', 'ASC')->get_where('oradores',array('status'=>1, 'evento_id'=>$this->evento_id))->result();
        return $result;
    }
                            ...
                            switch($values['medio_pago']) {
                                ...
                                case 'pago_mis_cuentas':
                                    ...
                                    if($email){
                                        ...
                                        $responseType = 'function';
                                        $function     = 'appendFormMessagesModal';
                                        $messages     = $this->view('alerts/modal_alerts', array('texto'=>'<li>Usted se ha pre-inscripto para participar de '.$this->getEvento()->nombre.'.<br>En breves instantes recibirá un email con todos los datos para pagar a través de Pago Mis Cuentas.<br/>Por favor revise su bandeja de SPAM.</li>', 'title'=>$this->getEvento()->nombre, 'class_type'=>'info'));
                                        $data = array('success' =>$success,'responseType'=>$responseType, 'html'=>$messages, 'value'=>$function, 'modal_redirect'=>lang_url());
                                    }
                                    break;
                                case 0:
                                    ...
                                    if($email){
                                        ...
                                        $responseType = 'function';
                                        $function     = 'appendFormMessagesModal';
                                        $messages     = $this->view('alerts/modal_alerts', array('texto'=>'<li>Usted se ha pre-inscripto para participar de '.$this->getEvento()->nombre.', Inspire Forum.<br><p class="text-center"><a class="btn btn-primary" href="'.lang_url('payments/latecheckout/'.$user_info->salt).'">REALIZAR PAGO</a></p></li>', 'title'=>$this->getEvento()->nombre, 'class_type'=>'info'));
                                        $data = array('success' =>$success,'responseType'=>$responseType, 'html'=>$messages, 'value'=>$function, 'modal_redirect'=>lang_url());
                                    }
                                    break;
                            }
