                            ...
                            switch($values['medio_pago']) {
                                ...
                                case 0:                                                                 
                                    ...
                                    if($this->cart->total() > 0) {
                                        $modal      = '<li>Usted se ha pre-inscripto para participar del evento '.$this->evento_name.'</li>';
                                        $subject    = "PreAcreditación ".$this->evento_name;
                                        $body       = $this->view('email/pago_no_procesado',array('user_info'=>$user_info, 'evento'=>$this->Evento->getEvento()));
                                    } else {
                                        $modal      = '<li>Usted se ha inscripto para participar del evento '.$this->evento_name.'</li>';
                                        $subject    = "Acreditación ".$this->evento_name;
                                        $body       = $this->view('email/cupon_100',array('user_info'=>$user_info, 'evento'=>$this->Evento->getEvento()));    
                                    }
                                    $email      = $this->Email->send('email_info', $user_email, $subject, $body);
                                    if($email){
                                        $success = true;
