                    ...
                    switch($tipo_usuario) {
                            ...
                            switch($values['medio_pago']) {
                                ...
                                case 'transferencia_bancaria':                                
                                    ...
                                    if($email){
                                        ...
                                        $responseType = 'function';
                                        $function     = 'appendFormMessagesModal';
                                        $messages     = $this->view('alerts/modal_alerts', array('texto'=>'<li>Usted se ha pre-inscripto para participar del evento '.$this->getEvento()->nombre.'.<br>Confirmando su  registro mediante el aporte del Bono Contribución a <b>BisBlick – Compromiso Social</b>, podemos confirmar la inscripción y asegurarle la entrada al auditorio principal.<br/>En breves instantes recibirá un email con todos los datos para realizar la transferencia.<br/>Por favor revise su bandeja de SPAM.</li>', 'title'=>$this->getEvento()->nombre, 'class_type'=>'info'));
                                        $data = array('success' =>$success,'responseType'=>$responseType, 'html'=>$messages, 'value'=>$function, 'modal_redirect'=>lang_url());
                                    }
                                    break;
                                ...
                                case 0:
                                    ...
                                    if($email){
                                        ...
                                        $responseType = 'function';
                                        $function     = 'appendFormMessagesModal';
                                        $messages     = $this->view('alerts/modal_alerts', array('texto'=>'<li>Usted se ha pre-inscripto para participar del evento '.$this->getEvento()->nombre.'.<br>Confirmando su  registro mediante el aporte del Bono Contribución a <b>BisBlick – Compromiso Social</b>, podemos confirmar la inscripción y asegurarle la entrada al auditorio principal.<br><p class="text-center"><a class="btn btn-primary" href="'.lang_url('payments/latecheckout/'.$user_info->salt).'">REALIZAR CONTRIBUCIÓN</a></p></li>', 'title'=>$this->getEvento()->nombre, 'class_type'=>'info'));
                                        $data = array('success' =>$success,'responseType'=>$responseType, 'html'=>$messages, 'value'=>$function, 'modal_redirect'=>lang_url());
                                    }
                                    break;                                    
                            }
                        ...
                        case 1:
                            ...
                            if($email) {                                        
                                ...
                                $responseType = 'function';
                                $function     = 'appendFormMessagesModal';
                                $messages     = $this->view('alerts/modal_alerts', array('texto'=>'<li class="">Como Antiguo Alumno Adherente del IAE, su inscripción al evento '.$this->getEvento()->nombre.' ha sido confirmada.<br>El evento es realizado a beneficio de <b>BisBlick - Compromiso Social</b>. Si desea contribuir para ayudar a personas de bajos recursos a terminar sus estudios superiores, presione aquí.<br><p class="text-center"><a class="btn btn-primary" href="'.lang_url('payments/latecheckout/'.$user_info->salt).'">REALIZAR CONTRIBUCIÓN</a></p></li>', 'title'=>$this->getEvento()->nombre, 'class_type'=>'info'));
                                $data = array('success' =>$success,'responseType'=>$responseType, 'html'=>$messages, 'value'=>$function, 'modal_redirect'=>lang_url());
                            }
                            break;
